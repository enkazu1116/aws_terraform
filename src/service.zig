...
/// フォルダ作成
/// 引数: 親フォルダ名、子フォルダ名
pub fn create_template_folder(parent_name: []const u8, child_name: []const u8) !void {
    const cwd = std.fs.cwd();

    // 親フォルダが無ければ作成する。既に存在していても問題なし。
    try cwd.makePath(parent_name);

    var parent = try cwd.openDir(parent_name, .{});
    defer parent.close();
...
/// ファイルテンプレート構成を作成する。
/// 引数: 親フォルダ名、子フォルダ名
pub fn create_template_files(parent_name: []const u8, child_name: []const u8) !void {
    ...
    var target_dir = try std.fs.cwd().openDir(rel_path, .{});
